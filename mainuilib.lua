...
if not isfolder("SteffsPrivate\\Themes") then
	pcall(makefolder, "SteffsPrivate")
	pcall(makefolder, "SteffsPrivate\\Themes")
end
...
for i, v in next, themes do
	if i ~= "selected" then
		writefile("SteffsPrivate\\Themes\\" .. i .. ".json", jsonEncodeTheme(v))
	end
end
...
function library.new(gameName)
	local lib = setmetatable({
		_name = gameName,
		_gui = create("ScreenGui", { Name = "SteffsPrivate", ZIndexBehavior = Enum.ZIndexBehavior.Sibling }, {
			create("Frame", { Name = "main", BackgroundColor3 = "theme.mainBackground", ClipsDescendants = true, Position = UDim2.new(0, 100, 0, 100), Size = UDim2.new(0, 535, 0, 380) }, {
				...
				create("Frame", { Name = "top", BackgroundColor3 = "theme.topBackground", Size = UDim2.new(1, 0, 0, 34), ZIndex = 2 }, {
					create("TextLabel", { Name = "title", BackgroundTransparency = 1, Font = Enum.Font.GothamSemibold, Position = UDim2.new(0, 34, 0, 0), Size = UDim2.new(1, -68, 1, 0), Text = "SteffsPrivate | " .. gameName, TextColor3 = "theme.textForeground", TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left }),
				...
				create("TextLabel", { Name = "site", AnchorPoint =  Vector2.new(0.5, 1), BackgroundTransparency = 1, Font = Enum.Font.GothamSemibold, Position = UDim2.new(0.5, 0, 1, 0), Size = UDim2.new(1, -16, 0, 26), Text = "", TextColor3 = "theme.textForeground", TextSize = 12, TextXAlignment = Enum.TextXAlignment.Right }),
...
function library:AddProfile()
	local newProfile = setmetatable({
		_frame = create("Frame", { Name = "profile", BackgroundColor3 = "theme.panelBackground", Parent = self._gui.main.dashboard }, {
			create("UISizeConstraint", { Name = "uisizecon", MinSize = Vector2.new(315, 104) }),
			create("ImageLabel", { Name = "icon", BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 10), Size = UDim2.new(0, 40, 0, 40) }),
			create("TextLabel", { Name = "name", BackgroundTransparency = 1, Font = Enum.Font.GothamSemibold, Position = UDim2.new(0, 65, 0, 15), Size = UDim2.new(1, -75, 0, 30), Text = "Welcome, " .. localPlayer.DisplayName, TextColor3 = "theme.textForeground", TextSize = 16, TextXAlignment = Enum.TextXAlignment.Left }),
...
function library:GetConfigs()
	local configs = {}
	if isfolder("SteffsPrivate\\Configs\\" .. self._name) then
		for i, v in next, listfiles("SteffsPrivate\\Configs\\" .. self._name) do
...
function library:LoadConfig(name)
	local filePath = "SteffsPrivate\\Configs\\" .. self._name .. "\\" .. name .. ".json"
	if not (isfolder("SteffsPrivate\\Configs\\" .. self._name) and isfile(filePath)) then
		return
	end
...
function library:SaveConfig(name)
	if not isfolder("SteffsPrivate\\Configs\\" .. self._name) then
		pcall(makefolder, "SteffsPrivate")
		pcall(makefolder, "SteffsPrivate\\Configs")
		pcall(makefolder, "SteffsPrivate\\Configs\\" .. self._name)
	end
	local config = {}
	for _, tab in next, self._tabs do
		for _, panel in next, tab._panels do
	...
	writefile("SteffsPrivate\\Configs\\" .. self._name .. "\\" .. name .. ".json", httpService:JSONEncode(config))
end
...
function library:GetThemes()
	...
	for i, v in next, listfiles("SteffsPrivate\\Themes") do
		local split = select(-1, unpack(v:split("\\")))
		if split:find(".json") then
...
function library:LoadTheme(name)
	local filePath = "SteffsPrivate\\Themes\\" .. name .. ".json"
	if not (isfolder("SteffsPrivate\\Themes") and isfile(filePath)) then
		print("noes")
		return
	end
...
function library:AddSettings()
	...
	credits:AddClipboardLabel("Steff - Creator", "https://discord.gg/bJhW8hBgkE")
...
function library:Notify(text, callback, options)
	...
	local frame = create("Frame", { Name = "notification", AnchorPoint = Vector2.new(1, 1), BackgroundColor3 = "theme.panelItemBackground", ClipsDescendants = true, Parent = self._gui.notifs, Position = UDim2.new(1, 300, 1, -30), Size = UDim2.new(0, 280, 0, sizeY + 34) }, {
		create("TextLabel", { Name = "title", BackgroundTransparency = 1, Font = Enum.Font.GothamSemibold, Position = UDim2.new(0, 10, 0, 0), Size = UDim2.new(1, -66, 0, 30), Text = "Notification", TextColor3 = "theme.textForeground", TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left }),
